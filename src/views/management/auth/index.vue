<template>
  <n-space vertical :size="12">
    <n-space>
      <n-button @click="downloadCsv">导出 CSV（原始数据）</n-button>
      <n-button @click="exportSorterAndFilterCsv">导出 CSV（展示的数据）</n-button>
    </n-space>
...
const columns: DataTableColumns<RowData> = [
  ...
  },
  {
    ...
    filter: (value: string | number, row: object) => {
      return Boolean((row as Song).address.indexOf(value as string));
    }
  }
];
